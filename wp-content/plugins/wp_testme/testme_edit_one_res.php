...
// Вывод возможных результатов для облегчения участи создающих и проверяющих
	...
	$testme_abc_points = $wpdb->get_results("SELECT a.answer_points FROM {$wpdb->prefix}testme_answers a, {$wpdb->prefix}testme_questions q 
	...
	(SELECT min(ROUND(a.answer_points)) AS min_points, max(ROUND(a.answer_points)) AS max_points FROM {$wpdb->prefix}testme_answers a, {$wpdb->prefix}testme_questions q 
