...
function mtq_update_status(mtqid){
	...
	mtq_have_completed_string = "Ви відповіли вірно на";
	mtq_question_string = "запитань";
	...
	mtq_your_score_is_string = "Ви набрали балів: ";
	if ( mtq_answer_display[mtqid] == 2 ){
		status_msg+="<br>"+ mtq_your_score_is_string + " " +mtq_score_percent[mtqid].toFixed(0)+" з "+mtq_max_score[mtqid];
	}
...
function mtq_button_click (q,a,mtqid)
{
		...
		for (j=1; j<=number_answers; j++ ){
			if ( parseInt(jQuery("#mtq_was_selected-"+q+"-"+j+"-"+mtqid).val() ) ){
				...
				//	jQuery("#mtq_button-"+q+"-"+j+"-"+mtqid).css('display','none');
					jQuery("#mtq_button-"+q+"-"+j+"-"+mtqid).addClass("selected-question");
				}
	...
	//hide questions after send correct
	if((num_attempts+1) >= number_correct){
		jQuery("#mtq_question-"+q+"-"+mtqid).slideUp(100);
	}
