...
?>
			...
			?>
			<div class="post-img">
                            <a href="<?php echo get_permalink($prev_post->ID); ?>"><img src="<?php echo mom_post_image('small-wide', $prev_post->ID); ?>" data-hidpi="<?php echo mom_post_image('small-wide-hd', $prev_post->ID); ?>" alt="<?php the_title(); ?>"></a>
                        </div>
			<?php } ?>
                        <div class="details <?php echo $is_img; ?>">
                            <a href="<?php echo get_permalink($prev_post->ID); ?>" class="link prev"><?php echo $ppa; ?><?php _e('Попередня', 'theme'); ?></a>
		...
		?>
			...
			?>
			<div class="post-img">
                            <a href="<?php echo get_permalink($next_post->ID); ?>"><img src="<?php echo mom_post_image('small-wide', $next_post->ID); ?>" data-hidpi="<?php echo mom_post_image('small-wide-hd', $next_post->ID); ?>" alt="<?php the_title(); ?>"></a>
                        </div>
			<?php } ?>
                        <div class="details <?php echo $is_img; ?>">
                            <a href="<?php echo get_permalink($next_post->ID); ?>" class="link next"><?php echo $npa; ?><?php _e('Наступна', 'theme'); ?></a>
...
<?php if ($style != 'min') { ?>
<?php if ($title != false) { ?>
<h2 class="single-title"><?php _e('Про автора', 'theme'); ?></h2>
<?php } ?>
<?php } ?>
            <div class="base-box single-box about-the-author">
                <div class="author_avatar"><?php echo get_avatar( get_the_author_meta( 'user_email', $id ), '80' ); ?></div>
                <div class="author_desc">
                    <h3 calss="vcard author"><span class="fn"><a href="<?php echo get_author_posts_url( $uid ); ?>"><?php the_author_meta( 'display_name', $id ); ?></a></span> <?php if ($style == 'min') { if (mom_option('author_np') == 1) { ?><span class="articles-count"><?php _e('Публікацій', 'theme'); ?> <?php echo number_format_i18n( get_the_author_posts() ); ?><?php } } ?> </span></h3>
        <p>
		<?php
...
if ( comments_open() ) {
	if ( $num_comments == 0 ) {
		$comments = __('Немає коментарів', 'theme');
	} elseif ( $num_comments > 1 ) {
		$comments = $num_comments .' '. __(' Коментарів', 'theme');
	} else {
		$comments = __('1 Коментар', 'theme');
	}
...
if($categories){
	foreach($categories as $category) {
		$cats.= '<a href="'.get_category_link( $category->term_id ).'" title="' . esc_attr( sprintf( __( "Переглянути всі новини з %s", 'theme' ), $category->name ) ) . '">'.$category->cat_name.'</a>'.$separator;
	}
}
	$output = '<div class="mom-post-meta '.$class.'">';
	$author = mom_option('post_meta-author') == 1 ? '<span class="author vcard">'.__('Автор:', 'theme').' <span class="fn"><a href="'.$author_link.'">'.get_the_author_meta( 'display_name' ).'</a></span></span>': '';
	$date = mom_option('post_meta-date') == 1 ? '<span><time datetime="'.get_the_time('c').'" itemprop="datePublished" class="updated">'.get_mom_date_format().'</time></span>': '';
	...
	$cat = mom_option('post_meta-cat') == 1 ? '<span>'.trim($cats, $separator).'</span>': '';
	...
	if(function_exists('the_views')) {
		$output .= '<span>'.__('Переглядів:', 'theme').' '.the_views(false).'</span>';
	}
...
function mom_related_posts () { ?>
                <h2 class="single-title"><?php _e('Схожі публікації', 'theme'); ?></h2>
...
<?php endwhile; ?>
                <?php  else:  ?>
                <h5><?php echo __('Немає схожих публікацій.', 'theme'); ?></h5>
                <?php  endif; ?>
                <?php wp_reset_query(); ?>
                  <?php endif;?>
<?php } else { ?>
...
<div class="no-display-in-front">
	<h3>Сторінка налаштування профілю</h3>

	<table class="form-table">

		<tr>
			<th><label for="ab_bg"><?php _e('Фон блоку автора', 'theme'); ?></label></th>
...
<br />
				<span class="description"><?php _e('Завантажити фон блоку автора.', 'theme'); ?></span>
			</td>
		</tr>

		<tr>
			<th><label for="ab_bg"><?php _e('Структура публікацій'); ?></label></th>

			<td>
				<select name="ap_l" id="ap_l">
					<option value=""><?php _e('Оберіть структуру ...', 'theme'); ?></option>
...
?>
					...
					<div class="mom-username">
						<input type="text" name="log" value="" placeholder="<?php _e('логін', 'theme'); ?>">
					</div>

					<div class="mom-password">
						<input type="password" name="pwd" value="" placeholder="<?php _e('пароль', 'theme'); ?>">
					</div>

					<div class="mom-submit-wrapper">
						<button class="button submit user-submit" name="user-submit" type="submit"><?php _e('Вхід', 'theme'); ?></button>
						<input type="checkbox" id="rememberme" name="rememberme" value="forever" <?php checked( 'rememberme', 1 ); ?>>
						<label for="rememberme"><i class="dashicons dashicons-yes"></i><?php _e('Запам\'ятати', 'theme'); ?></label>
                                                <input type="hidden" name="redirect_to" value="<?php echo esc_url( $redirect ); ?>" />
					</div>
					<?php if ($register != '' || $reset != '') { ?>
					<span class="mlw-extra">
						<?php if ($register != '') { ?>
						<a href="<?php echo $register; ?>" class="mlw-register"><?php _e('Реєстрація', 'theme'); ?></a>
						<?php } ?>
						<?php if ($reset != '') { ?>
						<a href="<?php echo $reset; ?>" class="mlw-reset"><?php _e('Забули пароль?', 'theme'); ?></a>
						<?php } ?>
					</span>
					<?php } ?>
                            </form>
				...
				<div class="lw-user-info">
					<a href="<?php echo get_author_posts_url( get_current_user_id() ); ?>"><?php _e('Привіт', 'theme'); ?>, <strong><?php echo $name; ?></strong></a>
					<a href=" <?php echo get_edit_profile_url($id); ?> " class="button"><?php _e('Редагувати', 'theme'); ?></a>
					<a href="<?php echo wp_logout_url(); ?>" class="button"><?php _e('Вихід', 'theme'); ?></a>
				</div>
...
?>
					...
					<label for="mom-login-input"><?php _e('логін', 'theme'); ?> <span class="required">*</span></label>
					<input type="text" name="log" id="mom-login-input" value="" placeholder="">
					</p>
					<p class="form-row form-password">
					<label for="mom-pwd-input"><?php _e('пароль', 'theme'); ?> <span class="required">*</span></label>
					...
					<div class="mom-submit-wrapper">
						<button class="button submit user-submit" name="user-submit" type="submit"><?php _e('Вхід', 'theme'); ?></button>
						<input type="checkbox" id="rememberme" name="rememberme" value="forever" <?php checked( 'rememberme', 1 ); ?>>
						<label for="rememberme"> <?php _e('Запам\'ятати', 'theme'); ?></label>
                                                <input type="hidden" name="redirect_to" value="<?php echo esc_url( $redirect ); ?>" />
					</div>
					<?php if ($register != '' || $reset != '') { ?>
					<p class="mlw-extra">
				...
				<div class="mom-login-user">
					...
					<div class="lw-user-info">
						<h4><a href="<?php echo get_author_posts_url( get_current_user_id() ); ?>"><?php _e('Привіт', 'theme'); ?>, <strong><?php echo $name; ?></strong></a></h4>
...
<form method="POST" id="adduser" class="user-forms" action="">
	<p class="form-username">
		<label for="user_name"><?php _e('Логін', 'theme'); ?> <span class="required">*</span></label>
		<input class="text-input" name="user_name" type="text" id="user_name" value="" />
	</p>
	
	<p class="form-email">
		<label for="email"><?php _e('E-mail', 'theme'); ?> <span class="required">*</span></label>
		<input class="text-input" name="email" type="text" id="email" value="" />
	</p>
	
	<p class="form-password">
		<label for="password"><?php _e('Пароль', 'theme'); ?> <span class="required">*</span></label>
		<input class="text-input" name="password" type="text" id="password" value="" />
	</p>
	
	<p class="form-submit">
		<input name="adduser" type="submit" id="addusersub" class="submit button" value="Реєстрація" />
...
function mom_registration_process_hook() {
	if (isset($_POST['adduser']) && isset($_POST['add-nonce']) && wp_verify_nonce($_POST['add-nonce'], 'add-user')) {
		...
		if ( !wp_verify_nonce($_POST['add-nonce'],'add-user') ) {
			...
			if ( !$userdata['user_login'] )
				$error = 'Поле логін обовязкове для реєстрації.';
			elseif ( username_exists($userdata['user_login']) )
				$error = 'Вибачте, введений логін вже існує!';
			elseif ( !is_email($userdata['user_email']) )
				$error = 'Ви повинні ввести справжню адресу e-mail.';
			elseif ( email_exists($userdata['user_email']) )
				$error = 'Вибачте, введена e-mail адреса вже використовується!';
	...
	<?php
		...
		echo 'Дякуємо за реєстрацію ' . $user->user_login;
		echo '<br/>Будь ласка, перевірте вашу e-mail адресу. У ній ви отримаєте свій логін та пароль для входу.<br/> (Можливо, що лист потрапить у спам)';
	?>
	</p>
