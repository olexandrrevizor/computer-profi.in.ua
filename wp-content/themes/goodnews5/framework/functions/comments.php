...
function mom_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class('single-comment base-box'); ?> id="comment-<?php comment_ID(); ?>">
		<p><?php _e( 'Pingback:', 'theme' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( '(Редагувати)', 'theme' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
	...
	<li <?php comment_class('single-comment'); ?> id="li-comment-<?php comment_ID(); ?>">
		...
		<?php if (mom_option('post_cn') == 1) { echo momcn_comment_numbering($comment->comment_ID, $args); }  ?>
			<?php if ( '0' == $comment->comment_approved ) : ?>
				<em class="comment-awaiting-moderation"><?php _e( 'Ваш коментар відправлено на модерацію.', 'theme' ); ?></em>
			<?php endif; ?>
				<?php
		...
		<div class="comment-content">
			...
			<?php
					printf( '<span class="comment-meta commentmetadata "><a href="%1$s"><time datetime="%2$s">%3$s</time></a></span>',
                        ...
                        <?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Відповісти', 'theme' ), 'after' => '', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
			<?php edit_comment_link( __( 'Редагувати', 'theme' )); ?>
			</div>
